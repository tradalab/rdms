...
import { GridInput } from "@/app/_components/grid-input"
import { useTranslation } from "react-i18next"

export function KeyAddValueSet({ form }: { form: UseFormReturn }) {
    ...
    <div className="rounded-lg border shadow-sm">
      <div className="bg-muted/50 text-muted-foreground grid grid-cols-[1fr_auto] divide-x divide-border rounded-t-lg">
        <div className="px-2.5 py-2 text-xs font-medium">
          <label id="set-value-value">{t("value")}</label>
        </div>
      ...
      {fields.map((_, index) => {
        return (
          ...
          >
            <FormField
              control={form.control}
              name={`value_set.${index}`}
              render={({ field: { value, ...field } }) => {
                return (
                  <FormItem>
                    <FormControl>
                      <GridInput {...field} aria-labelledby="set-value-value" placeholder="Value" />
                    </FormControl>
                  </FormItem>
                )
              }}
